...
def clear_questions():
    ...
    return "All Your Questions Deleted"
...
def leaderboard_data():
    ...
    cursor.execute("SELECT regno,score FROM results WHERE admin_id=%s ORDER BY score DESC", (session.get('admin_id'),))
    data=cursor.fetchall()
...
def upload_users():
    ...
    for i, row in df.iterrows():
        cursor.execute("INSERT INTO users (regno, password,admin_id) VALUES (%s, %s, %s)", (str(row['regno']), str(row['password']), session['admin_id']))

    conn.commit()
    conn.close()
    return "Users uploaded successfully"

@app.route('/generate_exam_code')
def generate_exam_code():
    ...
    cursor.execute("""
        INSERT INTO exam_codes (code, admin_id)
        VALUES (%s, %s)
    """, (code, session['admin_id']))

    conn.commit()
    conn.close()

    return jsonify({"exam_code": code})


@app.route('/verify_exam_code', methods=['POST'])
def verify_exam_code():

    code = request.form['exam_code'].strip()

    conn = get_db()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT admin_id FROM exam_codes
        WHERE code=%s
        AND is_active=1
    """,(code,))
    ...
    if exam:
        ...
        student_regno = session.get('user')
        cursor.execute("""SELECT admin_id FROM users WHERE regno=%s""", (student_regno,))
        student = cursor.fetchone()
        

        if student and student[0] == exam[0]:
            session['exam_code'] = code
            session['student_admin_id'] = exam[0]

            cursor.execute("""UPDATE users SET admin_id=%s WHERE regno=%s AND admin_id IS NULL""", (exam[0],student_regno))
            conn.close()
...
def upload():
    ...
    for i, row in df.iterrows():
        cursor.execute("""
        INSERT INTO questions(question, option1, option2, option3, option4, answer,admin_id)
...
def student_dashboard():
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM results WHERE regno=%s",
        (regno,)
    )
    attempts = cursor.fetchone()[0]

    cursor.execute(
        "SELECT MAX(score) FROM results WHERE regno=%s",
        (regno,)
    )
    best_score = cursor.fetchone()[0] or 0

    cursor.execute(
        "SELECT AVG(score) FROM results WHERE regno=%s",
        (regno,)
    )
...
def submit():
    ...
    cursor.execute("INSERT INTO results (regno, score, show_key, admin_id) VALUES (%s,%s,0,%s)", (regno,score,session.get('student_admin_id')))
    conn.commit()
    conn.close()

    session.pop('user', None)
    session.pop('exam_code', None)
